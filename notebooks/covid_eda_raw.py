...
#filter by country
df_algeria=df[df['entity']=='Algeria']
display(df_algeria)

#pivot data with indicator and fill in zeros for NAs

wide_df=df_algeria.pivot(columns='indicator', index=['entity', 'date'], values='value')
...
fig = px.line(wide_df.sort_values(by=['date'], ascending=[True]).sample(n=10000, replace=False, random_state=123).sort_index(), x='date', y=['Daily ICU occupancy', 'Daily ICU occupancy per million'])
fig
